...
data0['pred'] =  np.exp(model.predict(data2))

model = load_model("./elu_SGD_final/1_best_model.h5")
data0['pred_1m'] =  np.exp(model.predict(data2))

model = load_model("./elu_SGD_final/2_best_model.h5")
data0['pred_2m'] =  np.exp(model.predict(data2))

model = load_model("./elu_SGD_final/3_best_model.h5")
data0['pred_3m'] =  np.exp(model.predict(data2))

model = load_model("./elu_SGD_final/4_best_model.h5")
data0['pred_4m'] =  np.exp(model.predict(data2))
...
data0['pred_6m'] =  np.exp(model.predict(data2))

print(data0[['pred', 'pred_1m', 'pred_2m', 'pred_3m', 'pred_4m', 'pred_5m', 'pred_6m']])
